import type { WeatherLocationResolution } from "../../../packages/ai-core/src/aiContracts.js"

export interface BrazilStateEntry {
  name: string
  aliases: string[]
}
...
const BRAZIL_STATE_DEFINITIONS: BrazilStateEntry[] = [
...
function normalizeWeatherLookupKey(value = ""): string {
  return String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function detectBrazilStateDefinition(value = ""): BrazilStateEntry | null {
...
function isAmbiguousBrazilStateName(entry: BrazilStateEntry | null = null): boolean {
  const normalizedName = normalizeWeatherLookupKey(entry?.name || "")
  return normalizedName === "sao paulo" || normalizedName === "rio de janeiro"
}

function questionExplicitlyRequestsState(question = ""): boolean {
  return /\b(estado|uf)\b/.test(normalizeWeatherLookupKey(question))
}

function detectWeatherLocationScope(question = "", query = ""): "country" | "state" | "city" | "place" {
  const normalizedQuestion = normalizeWeatherLookupKey(question)
  const normalizedQuery = normalizeWeatherLookupKey(query)

  if (/\b(pais|country|nacao)\b/.test(normalizedQuestion)) return "country"
  if (/\b(estado|uf)\b/.test(normalizedQuestion)) return "state"
  if (/\b(cidade|municipio|capital)\b/.test(normalizedQuestion)) return "city"

  const stateDefinition = detectBrazilStateDefinition(normalizedQuery)
  if (stateDefinition && (!isAmbiguousBrazilStateName(stateDefinition) || questionExplicitlyRequestsState(question))) return "state"

  if (/^(brasil|brazil)$/.test(normalizedQuery)) return "country"
  return "place"
}

function classifyWeatherLocationResult(result: GeocodingResult = {}): "natural_feature" | "state" | "municipality" | "country" | "city" | "place" {
  const featureCode = normalizeWeatherLookupKey(result?.feature_code || result?.featureCode || "")
  if (/^(bay|gulf|sea|ocean|strait|channel|fjord|reef|shoal|lagoon|lake|reservoir|peninsula|cape|hill|mountain|peak|valley|plain|forest|desert|island|isthmus|waterfall|stream|river)$/.test(featureCode)) return "natural_feature"
...
function buildWeatherLookupParts(query = ""): string[] {
  return String(query || "")
    .split(",")
...
export function shouldRestrictWeatherLookupToBrazil(query = "", question = ""): boolean {
  const normalizedQuery = normalizeWeatherLookupKey(query)
  if (/\b(brasil|brazil)\b/.test(normalizedQuery)) return true

  const stateDefinition = detectBrazilStateDefinition(query)
  if (stateDefinition && (!isAmbiguousBrazilStateName(stateDefinition) || questionExplicitlyRequestsState(question))) return true

  return false
}

function matchesBrazilStateCandidate(result: GeocodingResult = {}, stateDefinition: BrazilStateEntry | null = null): boolean {
...
function buildWeatherLocationLabel(result: GeocodingResult = {}): string {
  ...
  if (!name) return [admin1, country].filter(Boolean).join(", ").trim()
  if (admin1 && normalizedAdmin1 && normalizedAdmin1 !== normalizedName) return [name, admin1].filter(Boolean).join(", ").trim()
  return [name, country].filter(Boolean).join(", ").trim()
}

function buildWeatherLocationDisplayLabel(result: GeocodingResult = {}, query = "", question = ""): string {
  ...
  if (locationType === "country" && name) return `Pais: ${name}`
  return buildWeatherLocationLabel(result) || sanitizeWeatherLocationQuery(query)
}

function scoreWeatherLocationCandidate(result: GeocodingResult = {}, { query = "", question = "" }: { query?: string, question?: string } = {}): number {
  const normalizedQuery = normalizeWeatherLookupKey(query)
  ...
  if (/\bbrasil\b/.test(String(query || "").toLowerCase()) && countryCode === "br") score += 16
  if (stateDefinition && resultType === "natural_feature") score -= 180

  const latitude = Number(result?.latitude)
  const longitude = Number(result?.longitude)
  if (Number.isFinite(latitude) && Number.isFinite(longitude)) score += 4

  return score
}

export function selectPreferredWeatherLocationResult(
  results: GeocodingResult[] = [],
  { query = "", question = "" }: { query?: string, question?: string } = {}
...
export function buildResolvedWeatherLocation(
