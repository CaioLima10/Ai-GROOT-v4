...
export function getImageGenerationToken() {
...
export function normalizeImageDimension(value, fallback) {
...
export function normalizeImageStylePreset(value = "") {
...
export function normalizeImageSeed(value) {
  if (value == null || value === "") return null
  const numeric = Math.trunc(Number(value))
  return Number.isFinite(numeric) ? Math.max(0, numeric) : null
}

export function resolveImageDimensions({ width, height, aspectRatio } = {}) {
...
export function buildImageStylePresetPrompt(stylePreset = "") {
  return IMAGE_STYLE_PRESETS[stylePreset] || ""
}

export function parseImageGenerationRequest(body = {}) {
