...
type Tier = "owner" | "paid" | "auth" | "anonymous"
...
type Feature = "upload" | "image"

const uploadQuotaState = new Map<string, QuotaState>()

function normalizePlan(raw = ""): "paid" | "auth" | "anonymous" {
...
export function resolveUploadQuotaContext(req: any): QuotaContext {
...
function getFeatureConfig(feature: Feature = "upload") {
  if (feature === "image") {
    return {
      feature,
...
export function resolveImageQuotaContext(req: any): QuotaContext {
...
export function getTierLimit(tier: Tier = "anonymous", feature: Feature = "upload"): number {
...
function getQuotaStatus(ctx: QuotaContext, feature: Feature = "upload") {
  const config = getFeatureConfig(feature)
  const now = Date.now()
...
function consumeQuota(ctx: QuotaContext, feature: Feature = "upload") {
  const config = getFeatureConfig(feature)
  const limit = getTierLimit(ctx.tier, feature)
  if (!Number.isFinite(limit)) {
...
export function getUploadQuotaStatus(ctx: QuotaContext) {
  return getQuotaStatus(ctx, "upload")
}

export function consumeUploadQuota(ctx: QuotaContext) {
  return consumeQuota(ctx, "upload")
}

export function getImageQuotaStatus(ctx: QuotaContext) {
  return getQuotaStatus(ctx, "image")
}

export function consumeImageQuota(ctx: QuotaContext) {
