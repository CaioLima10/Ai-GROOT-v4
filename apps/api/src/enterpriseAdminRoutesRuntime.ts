...
type TraceStoreLike = {
  getRecentTraces: (input: Record<string, unknown>) => unknown
  getSummary: () => unknown
}
...
type ToolRegistryLike = {
  listTools: () => unknown
  getSummary: () => unknown
  getRecentExecutions: (limit: number) => unknown
}

type JobManagerLike = {
  getSummary: () => unknown
  listJobs: (input: Record<string, unknown>) => unknown
  getJob: (jobId: string, input: { includePayload: boolean }) => unknown
}

type VoiceRuntimeLike = {
  getSummary: () => unknown
  listSessions: (limit: number) => unknown
}

type LongMemoryRuntimeLike = {
  getSummary: () => unknown
  getRecentCompactions: (limit: number) => unknown
  listRecentProfiles: (limit: number | string) => unknown
  listRecentSessions: (limit: number | string) => unknown
}

type LocalVoiceRuntimeLike = {
  getStatus?: () => unknown
}
...
type EnterpriseAdminRouteDeps = {
  ...
  voiceRuntime: VoiceRuntimeLike
  longMemoryRuntime: LongMemoryRuntimeLike
  localVoiceRuntime?: LocalVoiceRuntimeLike
}
...
function normalizeLimitValue(value: unknown, fallback: number) {
  const rawValue = normalizeSingleValue(value)
...
export function registerEnterpriseAdminRoutes(app: Express, deps: EnterpriseAdminRouteDeps) {
  ...
  app.get("/runtime/jobs/:jobId", requireAdmin, (req, res) => {
    try {
      const jobId = normalizeSingleValue(req.params.jobId)
      const job = jobId ? jobManager.getJob(jobId, { includePayload: true }) : null
      if (!job) {
  ...
  app.get("/runtime/memory/profiles", requireAdmin, (req, res) => {
    try {
      res.json({
        success: true,
        profiles: longMemoryRuntime.listRecentProfiles(normalizeLimitValue(req.query.limit, 25))
      })
    } catch {
  ...
  app.get("/runtime/memory/sessions", requireAdmin, (req, res) => {
    try {
      res.json({
        success: true,
        sessions: longMemoryRuntime.listRecentSessions(normalizeLimitValue(req.query.limit, 25))
      })
    } catch {
