...
/** @typedef {import("../../../packages/ai-core/src/aiContracts").LiveResearchRuntime} LiveResearchRuntime */
...
/**
 * @returns {LiveResearchRuntime}
 */
export function getLiveResearchRuntime() {
  return {
 ...
 * @returns {Promise<LiveFixtureSummary | null>}
 */
export async function resolveNextFixtureFromQuestion(question = "") {
  const fetchSportsDbJson = createSportsDbFetch()
  const searchTeamsByQuery = async (query) => {
  ...
  let nextMatches = structured.nextMatches.slice(0, 6)
  let recentMatches = structured.recentMatches.slice(0, 5)
  let liveMatches = Array.isArray(structured.liveMatches) ? structured.liveMatches.slice(0, 2) : []
