...
type UploadResponseDeps = {
  decodeXmlEntities: (text?: string) => string
  dedupeParagraphs: (paragraphs?: string[]) => string[]
  normalizeTextForDeduplication: (text?: string) => string
  isWeatherQuestion: (question?: string) => boolean
  isFixtureQuestion: (question?: string, context?: Record<string, unknown>) => boolean
}

const uploadQuestionStopWords = new Set([
  "que", "qual", "quais", "sobre", "arquivo", "imagem", "documento", "pdf", "doc", "docx", "ppt", "pptx", "xlsx", "planilha", "neste", "nesse", "deste", "dessa", "desse", "com", "para", "uma", "uns", "umas", "tem", "me", "mostre", "fale", "diga", "explique", "responda", "base", "somente", "nele", "nela", "dele", "dela"
])

function stripMarkupForSearch(text = "", deps: UploadResponseDeps): string {
...
function buildUploadSearchLines(extraction: UploadExtraction = {}, deps: UploadResponseDeps): string[] {
...
function extractUploadTaggedValue(lines: string[] = [], keys: string[] = [], deps: UploadResponseDeps): string {
...
function extractUploadNumberedItems(lines: string[] = []): string[] {
  return lines
    .map((line) => String(line || "").match(/^\d+[.)]\s*(.+)$/)?.[1]?.trim() || "")
    .filter(Boolean)
}

function parseUploadTableRows(lines: string[] = [], deps: UploadResponseDeps): TableRow[] {
...
function formatUploadSentence(value = "", prefix = ""): string {
...
function buildUploadExtractionUnavailableResponse(context: UploadContext = {}): string {
...
function pickBestUploadLine(question = "", lines: string[] = [], deps: UploadResponseDeps): string {
...
function buildUploadShortSummary(lines: string[] = [], deps: UploadResponseDeps): string {
...
function isGenericUploadOverviewQuestion(question = ""): boolean {
...
function buildUploadVisibleHighlights(lines: string[] = [], limit = 6): string[] {
  return lines
...
function buildDeterministicImageOverviewResponse(context: UploadContext = {}, lines: string[] = []): string {
  const extraction = context?.uploadExtraction || {}
...
function isUploadCapabilityQuestion(question = ""): boolean {
...
export function resolveDeterministicUploadResponse(question = "", context: UploadContext & Record<string, unknown> = {}, deps: UploadResponseDeps): string | null {
