...
export function formatFixtureDateLabel(date = "", time = "", timestamp = ""): string {
...
export function formatFixtureKickoff(date = "", time = "", timestamp = ""): string {
  const explicitTimestamp = String(timestamp || "").trim()
  if (explicitTimestamp) {
    const explicitDate = new Date(explicitTimestamp)
    if (!Number.isNaN(explicitDate.getTime())) {
...
function extractEventDateParts(event: Record<string, unknown>): EventDateParts {
...
function buildFixtureTimestamp(date = "", time = "", timestamp = ""): number {
...
export function isLiveFixtureStatus(value = ""): boolean {
  return normalizeLooseText(value) === "ao vivo"
}

function isClosedFixtureStatus(value = ""): boolean {
  return /^(encerrado|adiado)$/.test(normalizeLooseText(value))
}

export function isUpcomingFixtureCandidate(entry: Partial<LiveFixtureEntry> = {}, referenceNowMs = Date.now()): boolean {
  const timestampMs = Number(entry.timestampMs || 0)
  if (isLiveFixtureStatus(String(entry.status || ""))) {
    return timestampMs === 0 || timestampMs >= referenceNowMs - SPORTS_LIVE_LOOKBACK_MS
  }
  if (isClosedFixtureStatus(String(entry.status || ""))) {
    return false
  }
  if (timestampMs === 0) {
    return true
...
export function isRecentFixtureCandidate(entry: Partial<LiveFixtureEntry> = {}, referenceNowMs = Date.now()): boolean {
  const timestampMs = Number(entry.timestampMs || 0)
  if (isClosedFixtureStatus(String(entry.status || ""))) {
    return true
  }
...
export function sortUpcomingFixtures(left: Partial<LiveFixtureEntry> = {}, right: Partial<LiveFixtureEntry> = {}): number {
  const leftLive = isLiveFixtureStatus(String(left.status || ""))
  const rightLive = isLiveFixtureStatus(String(right.status || ""))
...
export function normalizeFixtureEvent(
...
interface BuildFixtureVerificationContext {
...
export function buildFixtureVerification(context: BuildFixtureVerificationContext = {}): FixtureVerification {
...
export function dedupeFixtures(items: LiveFixtureEntry[] = []): LiveFixtureEntry[] {
  const unique = new Map<string, LiveFixtureEntry>()
