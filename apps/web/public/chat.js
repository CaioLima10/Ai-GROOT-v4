...
const DEFAULT_UPLOAD_ACCEPT = "image/*,.pdf,.zip,.docx,.xlsx,.pptx,.txt,.md,.svg,.json,.jsonl,.js,.ts,.tsx,.jsx,.py,.java,.go,.rs,.c,.cpp,.h,.cs,.php,.rb,.html,.css,.xml,.yml,.yaml,.log,.tsv,.csv,.sql"
...
function bindEvents() {
  ...
  elements.scrollBottomBtn?.addEventListener("click", () => scrollChatToBottom(true))
...
function appendThinkingMessage(mode = "default") {
  const existingThinking = getChatContentNode().querySelector("article[data-thinking='true']")
  if (existingThinking) {
    removeThinkingMessage(existingThinking)
  }
...
function isUserNearChatBottom(threshold = CHAT_AUTO_SCROLL_THRESHOLD) {
  if (!elements.chat) return true
...
function syncScrollButton() {
  ...
  const remaining = elements.chat.scrollHeight - elements.chat.scrollTop - elements.chat.clientHeight
  elements.scrollBottomBtn.classList.toggle("hidden", remaining <= CHAT_AUTO_SCROLL_THRESHOLD)
}
...
async function sendMessage() {
  ...
  try {
    ...
    if (file) {
      renderFilePreview({ uploading: true })
    }

    let upload = null
    if (file) {
      try {
        upload = await uploadPendingFile(file)
      } finally {
...
async function uploadPendingFile(file) {
  ...
  const payload = await safeJson(response)
  if (!response.ok) {
    if (payload?.code === "UPLOAD_UNSUPPORTED_TYPE") {
      throw new Error(`Tipo não suportado para ${payload?.name || file.name}.`)
    }
    if (payload?.code === "UPLOAD_TYPE_MISMATCH") {
      throw new Error(`Arquivo inconsistente: ${payload?.name || file.name} (${payload?.detectedType || "tipo desconhecido"}).`)
    }
    if (payload?.code === "UPLOAD_TOO_LARGE") {
      throw new Error(`O anexo excede ${formatBytes(payload?.maxBytes || maxBytes)}.`)
    }
...
function getUploadFileIcon(file = {}) {
  ...
  if (mimeType.includes("word") || extension === ".docx") return "DOC"
  if (mimeType.includes("sheet") || extension === ".xlsx") return "XLS"
  if (mimeType.includes("presentation") || extension === ".pptx") return "PPT"
...
function renderFilePreview({ uploading = false } = {}) {
  ...
  const file = state.pendingFile
  const icon = getUploadFileIcon(file)
  ...
  elements.filePreview.innerHTML = `
    <div class="file-chip ${uploading ? "is-uploading" : ""}">
      ${preview}
      <div class="file-copy">
        <strong>${escapeHtml(file.name)}</strong>
        <span>${uploading ? "Enviando..." : formatBytes(file.size)}</span>
      </div>
      ${uploading ? "<span class=\"file-upload-spinner\" aria-hidden=\"true\"></span>" : ""}
      <button class="remove-file-btn" id="removePendingFileBtn" type="button" aria-label="Remover anexo">✕</button>
    </div>
  `

  const removeButton = document.getElementById("removePendingFileBtn")
  if (removeButton) {
...
function formatMessageSegment(segment = "") {
  ...
  const mediaCard = parseMediaLinkSegment(segment)
  if (mediaCard) {
    return renderMediaLinkSegment(mediaCard)
  }

  const dateTimeBlock = parseDateTimeSegment(segment)
  if (dateTimeBlock) {
    return renderDateTimeSegment(dateTimeBlock)
  }
...
function parseMediaLinkSegment(segment = "") {
  ...
  try {
    ...
    if (/youtube\.com|youtu\.be/.test(host)) {
      return {
        type: "video",
        label: "Video",
        title: "Link de video detectado",
        url: trimmed
      }
    }

    if (/google\.[a-z.]+$/.test(host) && /\/maps/.test(url.pathname)) {
      return {
        type: "map",
        label: "Mapa",
        title: "Link de mapa detectado",
        url: trimmed
      }
    }
...
function renderMediaLinkSegment(card = {}) {
  const label = card.type === "map" ? "Abrir mapa" : "Abrir video"
  return `
    <div class="rich-link-card" data-kind="${escapeHtml(card.type || "link")}">
      <div class="rich-link-meta">
        <span class="rich-link-badge">${escapeHtml(card.label || "Link")}</span>
        <strong>${escapeHtml(card.title || "Conteúdo detectado")}</strong>
        <small>${escapeHtml(card.url || "")}</small>
      </div>
      <a href="${escapeHtml(card.url || "")}" target="_blank" rel="noreferrer" class="message-document-action">${label}</a>
    </div>
  `
}

function parseDateTimeSegment(segment = "") {
  const lines = String(segment || "")
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean)

  if (lines.length < 1 || lines.length > 4) return null

  const matched = lines
    .map((line) => {
      const match = line.match(/^(data|hora|tempo|timezone|fuso)\s*[:\-]\s*(.+)$/i)
...
function renderDateTimeSegment(rows = []) {
  return `
    <div class="datetime-grid">
      ${rows.map((row) => `
        <div class="datetime-item">
          <span>${formatInlineContent(row.label)}</span>
          <strong>${formatInlineContent(row.value)}</strong>
        </div>
      `).join("")}
    </div>
  `
